...
try {
  const { resetPlayerCount, backupFile } = resetPlayerStats(loadGamePlugin().playerStatResetValues);
  const players = resetPlayerCount === 1 ? "player" : "players";
  console.log(`Reset time played, deaths and every game stat for ${resetPlayerCount} ${players}.`);
