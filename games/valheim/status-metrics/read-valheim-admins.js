...
const fs = require("fs");
const { ONLINE_PLAYERS_DIR } = require("../../../game-server/status-metrics/config");
...
function filterValheimAdminNames(onlinePlayerNames) {
  const adminSteamIds = readAdminSteamIds();
  if (adminSteamIds.size === 0) return [];

  return onlinePlayerNames.filter((name) => {
    try {
...
module.exports = { filterValheimAdminNames };
