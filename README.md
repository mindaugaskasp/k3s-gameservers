...
install/                   k3s.sh, monitoring.sh, vpa.sh (optional) -- run in order
monitoring-config/         Prometheus manifests
game-status-metrics/       player/status exporter sidecar (gamedig)
docs/                      setup-nodes, architecture, valheim, zomboid
...
2. Install the cluster:
   ```sh
   ./install/k3s.sh && ./install/monitoring.sh
   ```
   Grafana and Loki come from the servers-web repo.
...
- **Grafana:** `make dashboards` pushes to `GRAFANA_URL` (set in `.env`).
...
- [docs/architecture.md](docs/architecture.md): cluster, networking,
  monitoring, sizing
